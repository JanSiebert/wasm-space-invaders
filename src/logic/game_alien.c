...
static void alien_hit(GameObject *bullet, GameState *state, GameObject *alien) {
	...
	if (alien->alien_data.lives > 0) 
		...
		if (randn(12 - 2 * (5-state->player->player_data.lives)) == 0 && no_other_alien_below(alien, state)) {
...
GameObject *game_init_alien(GameState *state, int x, int y) {
	...
	alien->alien_data.lives = state->wave/4;
	if (alien->alien_data.lives > 3)
		alien->alien_data.lives = 3;
...
void game_init_alien_boss(GameState *state) {
	...
	alien->alien_data.shoot_prob = 100 - state->wave * 10;
