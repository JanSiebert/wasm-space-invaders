...
static void draw_status_panel(GameState *state) {
	...
	// Draw lives
	for (int i = 0; i < 5; i++) {
		spr(state->player->player_data.lives >i ? 6 : 7, 1, 1, SCREEN_WIDTH - (5*8) + i*8, 0);
	}
}
...
static void game_update_mainmenu(GameState *state) {
	...
	text("Space invaders", (SCREEN_WIDTH-14*(LETTER_WIDTH+1))/2, 25, 3);
	...
	text("(c) 2019 jan siebert", (SCREEN_WIDTH-20*(LETTER_WIDTH+1))/2, SCREEN_HEIGHT-8, 1);
